"""
Vercel serverless handler for Monday.com Proof Approved → Pouch Job Ticket webhook.
URL: /api/webhook_proof_approved

Triggered when the Proof Status column (status3__1) changes to "Proof Approved".
Finds the ProForma invoice on the Pricing board via PI#, extracts pouch specs
using Claude, fills the Pouch JT PDF template, and uploads it to the Job Ticket
column. Non-pouch invoices are silently skipped.
"""
import json
import sys
from pathlib import Path

# Project root (api/webhook_proof_approved.py -> root = parent directory)
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        _send_json(self, 200, {
            "status": "ok",
            "message": "Proof Approved webhook ready. POST with event data.",
        })
